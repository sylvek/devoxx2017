...
- le subscriber reçoit les messages que lorsqu'il écoute
...
A titre de comparaison avec le protocole HTTPS.
...
| Kafka | il est possible d'avoir un pont avec Kafka, mais qu'une partie du protocole est implémenté
| Amazon IoT | …
| Xively | …
|===
